...
export default function CameraPage() {
  ...
  const validateUrl = (url: string): boolean => {
    ...
    if (url.toLowerCase().startsWith("rtsp://")) {
      toast({
        title: "RTSP Not Supported",
        description:
          "Browsers can't play RTSP streams directly. Please convert to HLS using ffmpeg or a streaming server like MediaMTX.",
        variant: "destructive",
      });
      return false;
    }
    ...
    if (!url.toLowerCase().endsWith(".m3u8")) {
      toast({
        ...
        description: "Please provide an HLS stream URL (ending with .m3u8)",
        variant: "destructive",
      });
      return false;
    }
  ...
  const connectStream = () => {
    ...
    try {
      ...
      if (username || password) {
        // Use our proxy API to handle authentication
